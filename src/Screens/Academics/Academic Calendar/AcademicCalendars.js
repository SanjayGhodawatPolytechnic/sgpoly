...
const AcademicCalendars = () => {
    ...
    const getCalendarsSorted = (arr) => {
        if(arr){
        let sorted = arr.sort((a,b) => b.year.split("-")[0] - a.year.split("-")[0])
        return sorted
        }
        return []
    }
    ...
    const getAllCalendars = () => {
        ...
        dbRef.on("value", datasnapshot => {
            if(datasnapshot.val()){
                ...
                let cse = getCalendarsSorted(getFormatedObject(datasnapshot.val()["Computer Science"]))
                let mech = getCalendarsSorted(getFormatedObject(datasnapshot.val()["Mechanical"]))
                let cvl = getCalendarsSorted(getFormatedObject(datasnapshot.val()["Civil"]))
                let entc = getCalendarsSorted(getFormatedObject(datasnapshot.val()["E & TC"]))
                let ele = getCalendarsSorted(getFormatedObject(datasnapshot.val()["Electrical"]))
                let fy = getCalendarsSorted(getFormatedObject(datasnapshot.val()["Basic Science & Humanities"]))
    ...
    // useEffect(() => {
    //     console.log(currentlyDisplaying[0] && currentlyDisplaying[0].year)
    // }, [currentlyDisplaying])
    // useEffect(() => {
    ...
    return (
        ...
        >
            ...
            <div className="card-body">
                ...
                }} className="btn btn-info">Preview</button>
                <a href={cal.fileDownloadURL} target="blank" className="btn btn-info">Download</a>
            </div>
        </div>
                ))
            }
